# -*- coding:utf-8 -*-
# Last Change : Wed 19 Dec 2018 05:54:36.
...
class Client(object):
    ...
    def drawing_cv(self, mapsize=[1500, 1500], rot_deg=0, filename=None):
        ...
        def draw_map_overall(mapsize, rot_deg):
            ...
            for _line in range(0, 1500, 50):
                cv2.line(image, (0, _line), (mapsize[0], _line), (111,111,111), 2)
                cv2.line(image, (_line ,0), (_line, mapsize[1]), (111,111,111), 2)
            # 67-322
            cv2.rectangle(image, (pt0[0] + int(0 * self.block), pt0[1] - int(6  * self.block)), (pt0[0] + int(13 * self.block), pt0[1] - int(-15 * self.block)), self.color["BLUE"], thickness=3, lineType=cv2.LINE_AA)
            # 67-323
            cv2.rectangle(image, (pt0[0] + int(0 * self.block-2), pt0[1] - int(6  * self.block)), (pt0[0] + int(-13 * self.block), pt0[1] - int(-15 * self.block)), self.color["BLUE"], thickness=3, lineType=cv2.LINE_AA)
            cv2.rectangle(image, (0                             , pt0[1] - int(15 * self.block)), (mapsize[0]                   , pt0[1] - int(7   * self.block)), self.color["BLUE"], thickness=3, lineType=cv2.LINE_AA)
            # show sensing point
