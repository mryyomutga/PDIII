...
def generator(inputs, batch_size):
    ...
    乱数ベクトルから画像を生成する
    Args:
        inputs : [batch_size, 10]のTensor
        batch_size : 32
    Returns:
        生成結果の[batch_size, 32, 32, 1]のTensor
    """
...
def discriminator(inputs, reuse=False):
    """
    入力された画像が学習データであるか判別する
    Args:
        inputs : [batch_size, height(=32), weight(=32), channels]のTensor
        reuse : 変数再利用フラグ
    Returns:
        推論結果の[batch_size, 2]のTensor
    """
